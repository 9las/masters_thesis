...
# Make validation data into a dict of numpy arrays
df_validation = dict(df_validation)

for key, value in df_validation.items():
    df_validation[key] = np.stack(arrays = value)
...
for old_key, new_key in key_map:
    df_validation[new_key] = df_validation.pop(old_key)
...
# Do the prediction
data['prediction'] = model.predict(x = df_validation)

# Save predictions
data.to_csv(path_or_buf = '../data/s11_m{}_t{}v{}_predictions_on_validation.tsv'.format(model_index, t, v),
            sep = '\t',
            index = False)
